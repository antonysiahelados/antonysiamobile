...
(function () {
  ...
  /* Verifica una dirección real contra OpenStreetMap (Nominatim, sin API key)
     y devuelve si cae dentro del radio de reparto. */
  function verificarDireccion(dir) {
    var q = (dir || "").trim();
    if (q.length < 8) return Promise.resolve({ s: "idle" });
    var url = "https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=ar" +
      "&addressdetails=1&accept-language=es&q=" + encodeURIComponent(q + ", Buenos Aires, Argentina");
    return fetch(url, { headers: { Accept: "application/json" } })
      .then(function (r) { return r.json(); })
      .then(function (j) {
        if (!j || !j.length) return { s: "none" };
        var h = j[0], a = h.address || {};
        var lat = parseFloat(h.lat), lon = parseFloat(h.lon);
        var partes = [
          a.suburb || a.town || a.village || a.city_district || a.city,
          a.state_district || a.state
        ].filter(Boolean);
        var out = { lat: lat, lon: lon, label: partes.join(", ") || h.display_name, maps: mapsUrl(q + ", Buenos Aires, Argentina") };
        if (!a.house_number && !/\d{2,}/.test(q)) out.sinAltura = true;
        out.km = distanciaKm(CONFIG.local.lat, CONFIG.local.lon, lat, lon);
        out.s = out.sinAltura ? "vago" : (out.km <= CONFIG.radioKm ? "ok" : "far");
        return out;
      })
      .catch(function () { return { s: "err" }; });
  }
  ...
  function mensajeWhatsApp(o) {
    ...
    var env = o.retiro ? 0 : costoEnvio(sub);
    L.push("");
    if (o.promoLabel) L.push("\u{1F3F7} *Promo:* " + o.promoLabel + " (-" + ars(o.descuento) + ")");
    L.push(o.retiro ? "\u{1F3EA} *Retiro en el local* (sin cargo de envío)"
      : "\u{1F6F5} *Envío:* " + (env ? ars(env) : "Bonificado"));
    L.push("\u{1F4B0} *Total:* " + ars(sub + env));
    L.push("");
    L.push("\u{1F464} *Cliente:* " + (o.nombre || "A coordinar"));
    if (o.retiro) {
      L.push("\u{1F4CD} *Paso a buscarlo por:* " + CONFIG.direccion);
    } else {
      L.push("\u{1F4CD} *Dirección:* " + o.dir);
      if (o.mapsUrl) L.push("\u{1F5FA} " + o.mapsUrl);
    }
  ...
  window.ANTONYSIA = {
    ...
    mapsUrl: mapsUrl, distanciaKm: distanciaKm, verificarDireccion: verificarDireccion
  };
})();
